...
class EnIndexView(ListView):
    ...
    def get_context_data(self, **kwargs):
        context = super(EnIndexView, self).get_context_data(**kwargs)
        context.update({
            "current_section": self.get_current_section(),
            "search_term": self.search_term()
        })
...
class EnSectionIndexView(EnIndexView):
    def get_current_section(self):
        section = get_object_or_404(Section, slug__iexact=self.kwargs.get("section"))
        return section

    def get_queryset(self):
        queryset = super(EnSectionIndexView, self).get_queryset()
        queryset = queryset.filter(section__slug__iexact=self.kwargs.get("section"))
        return queryset
